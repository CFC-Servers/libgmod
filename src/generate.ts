...
const TYPE_NAME_OVERRIDES: { [key: string]: string } = {
    "vararg": "any ...",
    "Global": "_G",

};

const OVERLOADS: { [key: string]: string } = {
...
function getArgName(arg: FuncArg): string {
    ...
    let name = arg.name;
    for (const entry of KEYWORD_REPLACEMENTS) {
...
async function doGlobals(): Promise<void> {
    let data: Func[] = JSON.parse(
        await fs.readFile("parsed/global-functions.json", "utf-8")
    );

    data = _.sortBy(data, "name");
    await fs.writeFile("annotations/globals.lua", "", "utf-8");
    ...
    for (const func of data) {
        ...
        if (funcdata) {
            await fs.appendFile("annotations/globals.lua", funcdata + "\n", "utf-8");
        }
    }
...
async function doLibs(): Promise<void> {
    let data: FuncContainer[] = JSON.parse(
        await fs.readFile("parsed/libraries.json", "utf-8")
    );
    data = _.sortBy(data, "name");
    await fs.mkdir("annotations/libraries", { recursive: true });
    for (let lib of data) {
        let libdata: string;
        try {
        ...
        let filename = path.join("annotations", "libraries", `${lib.name}.lua`);
...
async function getPanels(): Promise<FuncContainer[]> {
    let data: FuncContainer[] = JSON.parse(
        await fs.readFile("parsed/panels.json", "utf-8")
    );
    return data;
}
...
async function doPanels(data: FuncContainer[]): Promise<void> {
    data = _.sortBy(data, "name");
    await fs.mkdir("annotations/panels", { recursive: true });
    for (let cls of data) {
        ...
        let filename = path.join("annotations", "panels", `${cls.name}.lua`);
...
async function getClasses(): Promise<FuncContainer[]> {
    let data: FuncContainer[] = JSON.parse(
        await fs.readFile("parsed/classes.json", "utf-8")
    );
    return data;
}
...
async function doClasses(data: FuncContainer[]): Promise<void> {
    data = _.sortBy(data, "name");
    await fs.mkdir("annotations/classes", { recursive: true });
    for (const cls of data) {
        ...
        let filename = path.join("annotations", "classes", `${cls.name}.lua`);
        await fs.writeFile(filename, classdata);
