...
function WorkshopFileBase:Fetch(type, offset, perpage, extratags, searchText, filter, sort)
end
...
--- @param filter? number @The filter for the results
--- @param sort? string @How it should be sorted
function WorkshopFileBase:FetchSubscribed(offset, perpage, tags, searchText, isUGC, filter, sort)
end
...
function WorkshopFileBase:FillFileInfo(results, isUGC)
end
...
function WorkshopFileBase:Publish(filename, image)
end
...
function WorkshopFileBase:RetrieveUserName(steamid, callback)
end
